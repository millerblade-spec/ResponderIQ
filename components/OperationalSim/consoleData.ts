 ...
 * scene dynamics, clinical, transport) and derive small view-models for the
 * console's left/right columns (Scenario Progress, Scene Status, Immediate
 * Priorities, Equipment/Retrieval, live Timeline). They add no new domain
 * state, dispatch no actions, and change no timing/scoring/persistence —
 * every value is computed from state the simulator machines already own.
 */
import type { SceneSafetyState, SceneSafetyStage } from '@/lib/opsim/sceneMachine';
...
import type { TransportState, TransportStage } from '@/lib/opsim/transportMachine';
...
const TRANSPORT_STAGE_LABELS: Record<TransportStage, string> = {
  device_choice: 'Choosing a packaging device',
  device_retrieval: 'Retrieving device from Medic 3',
  pelvic_choice: 'Pelvic support decision',
  pelvic_retrieval: 'Retrieving pelvic binder from Medic 3',
  securing: 'Securing patient to device',
  descending: 'Moving patient down',
  als_workup: 'ALS workup',
  stretcher_transfer: 'Stretcher transfer',
  pain_decision: 'Pain management decision',
  complete: 'Complete',
};

export function transportStageLabel(transport?: TransportState): string {
  return transport ? TRANSPORT_STAGE_LABELS[transport.stage] : 'Not yet started';
}

/**
 * The high-level run checklist for the Scenario Progress column. Dispatch,
 * differential, and parking are complete by the time the on-scene console
 * renders (the console only mounts once the OpSim stage is `ready`), so those
 * three read `done`; the remaining steps — including equipment, which now
 * happens inside the scene-safety flow rather than before the console mounts
 * (§9) — derive from live scene/crew/clinical/transport state.
 */
export function progressSteps(live: LiveConsoleState): ProgressStep[] {
  const { scene, crew, clinical, transport, ronArrived } = live;
  const safe = scene ? isClearedToEnter(scene) : false;
  const windshieldDone = scene?.windshieldReviewed ?? false;
  const equipmentDone = scene?.equipment.confirmed ?? false;
...
export function immediatePriorities(live: LiveConsoleState): Priority[] {
  const { scene, crew, dynamics, clinical, transport } = live;
  const out: Priority[] = [];
  ...
  if (transport && transport.stage !== 'complete' && clinicalCompleteCount(clinical) > 0) {
    out.push({
      id: 'transport_active',
      label: `Packaging & transport: ${transportStageLabel(transport)}`,
      tone: 'pending',
    });
  }
 ...
 * record (crew assignments, distraction actions, clinical actions, transport
 * events). This is a display convenience — the authoritative capture still
 * happens in lib/opsim/captureRun at completion.
 */
export function timelineEvents(live: LiveConsoleState): TimelineEvent[] {
  const events: TimelineEvent[] = [];
  const { crew, dynamics, clinical, scene, transport } = live;
  ...
  if (transport) {
    for (const e of transport.events) {
      events.push({ atSecond: e.atSecond, label: `Transport: ${humanizeId(e.id)}`, tone: 'info' });
    }
  }

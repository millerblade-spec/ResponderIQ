...
interface OnSceneOpsProps {
  ...
  /** Resolved display label for the parking choice, for the console's Scene status rail. */
...
/**
 * On-scene operations, presented as a command console over ONE shared crew
 * model (§19, §20). Owns the crew controller and clinical unlock, mirrors
 * each panel's live state up (for the console's status/priorities/scene-view
 * rails and for run capture), and completes the run automatically when
 * TransportOps reaches its pain-management decision (fix #16) — there is no
 * manual "end scenario" shortcut, so the mission cannot finish without the
 * full packaging → descent → ALS workup → stretcher transfer → pain-decision
 * sequence. Equipment arrives from the scene-safety flow's "what do you want
 * to bring in?" moment (§9) via `onEquipmentConfirmed`, never from a
 * top-level modal. All interactive panels stay mounted at all times — tabs
 * toggle visibility only — so mission-clock scheduling inside them is never
 * torn down.
 */
export function OnSceneOps({
  ...
  // The equipment the crew actually confirmed carrying in at the truck (§9) —
  // captured once, at confirmation, for run-facts scoring. This is distinct
  // from controller.crew.equipmentOnScene (below), which keeps growing as
  // retrievals land and is what the console displays live.
  const [equipmentSelected, setEquipmentSelected] = useState<readonly string[]>([]);

  // Live mirrors of each sub-machine's state, kept in React state so the
  // console rails re-render. Refs preserve the exact value read at completion
  // capture.
  ...
  // The pain-management decision (TransportOps' 'complete' stage) is the ONLY
  // path that ends the run — there is no header/console shortcut that can
  // finish the mission without it (fix #16).
  ...
  const live: LiveConsoleState = {
    ...
    equipmentOnScene: onSceneEquipment,
    now: nowSeconds,
  };
    ...
    <div className={styles.grid} data-tab={tab}>
        ...
        <section className={styles.miniPanel} aria-label="Scene status">
          ...
          <div className={styles.kvRows}>
            <div className={styles.kvRow}>
              <span className={styles.kvKey}>Parked</span>
              <span className={styles.kvVal}>{parkingLabel ?? '—'}</span>
            </div>
            <div className={styles.kvRow}>
      ...
      <div className={styles.colCenter}>
        ...
        <div className={styles.workspace}>
          ...
          <div className={styles.tabPane} role="tabpanel" hidden={tab !== 'transport'}>
            <TransportOps
              controller={controller}
              clock={clock}
              unlocked={clinicalUnlocked}
              model={clinicalModel}
              onStateChange={handleTransport}
      ...
      <div className={styles.bottomStrip}>
        <div className={styles.stripSections}>
          ...
          <section className={styles.miniPanel} aria-label="Disposition">
            ...
            <div className={styles.kvRows}>
              <div className={styles.kvRow}>
                <span className={styles.kvKey}>Transport stage</span>
                <span className={styles.kvVal}>{transportStageLabel(transportState)}</span>
              </div>
            </div>
            <p className={styles.prioritiesEmpty} style={{ marginTop: '0.6rem' }}>
              {transportState?.stage === 'complete'
                ? 'Run complete — handing off to the debrief.'
                : 'The run ends automatically once the Packaging & Transport tab reaches the pain-management decision — there is no separate "complete run" action.'}
            </p>
          </section>
        </div>
      </div>
